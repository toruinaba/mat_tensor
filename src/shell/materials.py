import numpy as np
import logging
from src.shell.utils import I_sh, Is_sh, IxI_sh, Id_s_sh, Id_sh, P
from src.axisymmetry.utils import I_ax, Is_ax, Id_s_ax, IxI_ax, Id_ax
from src.axisymmetry.materials import Elastic_ax, Material_expression_base_ax
...
class Elastic_sh(Elastic_ax):
    ...
    @property
    def De_sh(self):
        return 2 * self.G * Is_sh + self.A_sh * (self.K - 2 / 3 * self.G) * IxI_sh

    @property
    def De_sh_inv(self):
        return np.linalg.inv(self.De_sh)


class Material_expression_base_sh(Material_expression_base_ax):
    PS_I = 10

    def __init__(self, elastic: Elastic_sh, sig_y: float):
        super().__init__(elastic, sig_y)
        self.eps_e_33 = 0.0
        self.eps_e_33_i = 0.0

    def integrate_stress(self, eps, del_eps):
        eps_ax = np.array([eps[0], eps[1], eps[2], self.eps_e_33 + self.eps_p[3]])
        del_eps_ax = np.array([del_eps[0], del_eps[1], del_eps[2], 0.0])
        eps_tri = eps_ax + del_eps_ax
        eps_e_tri = eps_tri - self.eps_p
        for sh_itr in range(self.PS_I):
            ...
            if abs(sig[3]) < self.TOL:
                logger.info(f"Plane stress converged itr.{sh_itr+1}: sig_33 = {sig[3]}")
                self.eps_e_33_i = eps_e_tri[3]
                D11 = Dep[:3, :3]
                D12 = Dep[3, :3]
                D21 = Dep[:3, 3]
                D22 = Dep[3, 3]
                Dep_ps = D11 - np.outer(D12, D21) / D22
                return sig[:3], Dep_ps
            else:
                logger.info(f"itr.{sh_itr+1}: sig_33 = {sig[3]}")
                eps_e_tri[3] -= sig[3] / Dep[3, 3]
        else:
            raise NotConvergedError("Plane stress isn't converged")


class Linear_isotropic_sh(Material_expression_base_sh):
    def __init__(self, elastic: Elastic_sh, sig_y: float, h: float):
        super().__init__(elastic, sig_y)
        self.h = h
...
class Linear_kinematic_sh(Material_expression_base_sh):
    def __init__(self, elastic: Elastic_sh, sig_y: float, h: float):
        super().__init__(elastic, sig_y)
        self.h = h
...
class Voce_isotropic_sh(Material_expression_base_sh):
    def __init__(self, elastic: Elastic_sh, sig_y: float, Q: float, b: float):
        super().__init__(elastic, sig_y)
        self.Q = Q
        self.b = b
...
class AF_kinematic_sh(Material_expression_base_sh):
    def __init__(self, elastic: Elastic_sh, sig_y: float, C: float, k: float):
        super().__init__(elastic, sig_y)
        self.C = C
        self.k = k
...
class Yoshida_uemori_sh:
    TOL = 1.0e-06
    RM_I = 20
    Q = np.diag([1.0, 1.0, 2.0, 1.0])
    PS_I = 10

    def __init__(self, elastic: Elastic_sh, sig_y: float, B, C, Rsat, k, b, h, Ea, psi):
    ...
    def calc_j_f_ep(self, sig_d, sig_d_tri, eta, delta_gam):
        ...
        dn_dsig = (
            3
            / (2 * g_eta)
            * (I_ax - np.outer(n_s_f / np.sqrt(3 / 2), n_s_f / np.sqrt(3 / 2)))
        )
    ...
    def integrate_stress(self, eps, del_eps):
        ...
        del_eps_ax = np.array([del_eps[0], del_eps[1], del_eps[2], 0.0])
        eps_tri = eps_ax + del_eps_ax
        sig_i = self.De @ (eps_ax - self.eps_p)
        sig_d_i = Id_s_ax @ sig_i
        eps_e_tri = eps_tri - self.eps_p
        for sh_itr in range(self.PS_I):
            logger.info(f"Plane stress itr.{sh_itr+1}")
            ...
            if delta_gam == 0.0:
                if abs(sig_tri[3]) < self.TOL:
                    logger.info(
                        f"Plane stress converged itr.{sh_itr+1}: sig_33 = {sig_tri[3]}"
                    )
                    self.eps_e_33_i = eps_e_tri[3]
                    D11 = self.elastic.De[:3, :3]
                    D12 = self.elastic.De[3, :3]
                    D21 = self.elastic.De[:3, 3]
                    D22 = self.elastic.De[3, 3]
                    Dep_ps = D11 - np.outer(D12, D21) / D22
                    return sig_tri[:3], Dep_ps
                else:
                    logger.info(f"itr.{sh_itr+1}: sig_33 = {sig_tri[3]}")
                    eps_e_tri[3] -= sig_tri[3] / self.elastic.De[3, 3]
                    continue
...
class Yoshida_uemori_sh2:
    ...
    def __init__(self, elastic: Elastic_sh, sig_y: float, B, C, Rsat, k, b, h, Ea, psi):
        ...
        self.theta_i = np.zeros(3)
        self.beta = np.zeros(3)
        self.beta_i = np.zeros(3)
        self.R = 0.0
        self.R_i = 0.0
        self.r = 0.0
        self.r_i = 0.0
    ...
    @staticmethod
    def calc_stress_norm(sig):
        g = Yoshida_uemori_sh2.calc_g(sig)
        return np.sqrt(3 / 2 * g)
    ...
    def update_i(self, delta_beta, delta_theta, delta_gam, eta):
        flow = P @ eta
        g = self.calc_g(eta)
        d_eff_eps_p = delta_gam * np.sqrt(2 / 3 * g)
        self.beta_i = self.beta + delta_beta
        self.theta_i = self.theta + delta_theta
    ...
    def calc_j_f_f(self, eta):
        ...
        f_f_dtheta = -flow
        f_f_dgamma = [0.0]
        vectors = (f_f_dsig, f_f_dtheta, f_f_dbeta, f_f_dgamma)
        return np.hstack(vectors)

    def calc_f_ep(self, sig, eta, sig_tri, delta_gam):
        flow = P @ eta
        g = self.calc_g(eta)
        d_eff_eps_p = delta_gam * np.sqrt(2 / 3 * g)
        factor = self.calc_De_factor(
            self.Ea, self.elastic.E, self.psi, self.eff_eps_p + d_eff_eps_p
        )
        return 1 / factor * self.elastic.De_sh_inv @ (sig - sig_tri) + delta_gam * flow

    def calc_f_ep_dsig(self, eta, delta_gam):
        g = self.calc_g(eta)
        d_eff_eps_p = delta_gam * np.sqrt(2 / 3 * g)
        factor = self.calc_De_factor(
            self.Ea, self.elastic.E, self.psi, self.eff_eps_p + d_eff_eps_p
        )
        return 1 / factor * self.elastic.De_sh_inv + delta_gam * P

    def calc_f_ep_dtheta(self, eta, delta_gam):
        return -delta_gam * P

    def calc_f_ep_dbeta(self, eta, delta_gam):
        return -delta_gam * P

    def calc_f_ep_dgamma(self, sig, eta, sig_tri, delta_gam):
        g = self.calc_g(eta)
        d_eff_eps_p = delta_gam * np.sqrt(2 / 3 * g)
        factor = self.calc_De_factor(
            self.Ea, self.elastic.E, self.psi, self.eff_eps_p + d_eff_eps_p
        ...
        f_prime = (
            ...
            * (
                ...
                * np.sqrt(2 / 3)
                * g
                * (1 - self.Ea / self.elastic.E)
                * np.exp(-self.psi * (self.eff_eps_p + d_eff_eps_p))
            )
        )
        return f_prime * self.elastic.De_sh_inv @ (sig - sig_tri) + P @ eta

    def calc_j_f_ep(self, eta, delta_gam):
        f_ep_dsig = self.calc_f_ep_dsig(eta, delta_gam)
        f_ep_dbeta = -delta_gam * P
        f_ep_dtheta = -delta_gam * P
        f_ep_dgamma = P @ eta
        matrices = (
            f_ep_dsig,
            f_ep_dtheta,
    ...
    def calc_f_theta(self, eta, theta, a, delta_gam):
        theta_bar = self.calc_stress_norm(theta)
        if theta_bar == 0.0:
            return theta - self.theta - 2 / 3 * self.C * a * eta * delta_gam
        return (
            theta
            - self.theta
            - 2
            / 3
            * (self.C * a * eta - self.C * self.sig_y * np.sqrt(a / theta_bar) * theta)
            * delta_gam
        )

    def calc_f_theta_dsig(self, a, delta_gam):
        return -2 / 3 * self.C * a * delta_gam * I_sh

    def calc_f_theta_dtheta(self, eta, theta, a, delta_gam):
        theta_bar = self.calc_stress_norm(theta)
        theta_flow = P @ theta
        if theta_bar == 0.0:
            return (1 + 2 / 3 * self.C * a * delta_gam) * I_sh
        else:
            tb_theta = np.sqrt(3) * theta_flow / (2 * np.sqrt(2 * self.calc_g(theta)))
            f_theta_dtheta = (
                1
                + 2 / 3 * self.C * a * delta_gam
                + 2 / 3 * self.C * self.sig_y * delta_gam * np.sqrt(a / theta_bar)
            ) * I_sh - 2 * self.C * self.sig_y * delta_gam / (3 * theta_bar) * np.sqrt(
                a / theta_bar
            ) * np.outer(
                tb_theta, theta
            )
    ...
    def calc_f_theta_dgamma(self, eta, theta, a, delta_gam):
        theta_bar = self.calc_stress_norm(theta)
        s = 1 / (1 + 2 / 3 * self.k * self.sig_y * delta_gam)
        s_prime = -2 / 3 * self.k * self.sig_y * s * s
        a_prime = (
            s_prime * (self.R + 2 / 3 * self.k * self.sig_y * self.Rsat * delta_gam)
            + 2 / 3 * s * self.k * self.sig_y * self.Rsat
        )
        if theta_bar == 0.0:
            print("theta_bar is zero")
            f_theta_dgamma = (
                -2 / 3 * self.C * delta_gam * a_prime * eta - 2 / 3 * self.C * a * eta
            )
            return f_theta_dgamma
        else:
            f_theta_dgamma = (
                ...
                * self.C
                * (
                    eta * (a_prime * delta_gam + a)
                    - self.sig_y
                    * theta
                    * (
                        np.sqrt(a / theta_bar)
                        + delta_gam / (2 * np.sqrt(a * theta_bar))
                    )
                )
            )
            return f_theta_dgamma

    def calc_f_theta_dgamma_num(self, eta, theta, a, delta_gam):
        h = 1.0e-32
        delta_gam_comp = delta_gam + h * 1.0j
        s_comp = 1 / (1 + 2 / 3 * self.k * self.sig_y * delta_gam_comp)
        a_comp = (
            self.B
            + s_comp
            * (self.R + 2 / 3 * self.k * self.sig_y * self.Rsat * delta_gam_comp)
            - self.sig_y
        )
        f_theta_i = self.calc_f_theta(eta, theta, a_comp, delta_gam_comp)
        f_theta_dgamma_expected = np.imag(f_theta_i) / h
        return f_theta_dgamma_expected

    def calc_j_f_theta(self, eta, theta, a, delta_gam):
        theta_bar = self.calc_stress_norm(theta)
        theta_flow = P @ theta
        s = 1 / (1 + 2 / 3 * self.k * self.sig_y * delta_gam)
        s_prime = -2 / 3 * self.k * self.sig_y * s**2
        a_prime = (
            s_prime * (self.R + self.k * self.sig_y * self.Rsat * delta_gam)
            + s * self.k * self.sig_y * self.Rsat
        )
        f_theta_dsig = -2 / 3 * self.C * a * delta_gam * I_sh
        if theta_bar == 0.0:
            f_theta_dtheta = (1 + 2 / 3 * self.C * a * delta_gam) * I_sh
            f_theta_dgamma = (
                -2 / 3 * self.C * delta_gam * a_prime * eta - 2 / 3 * self.C * a * eta
            )
        else:
            ...
            f_theta_dgamma = self.calc_f_theta_dgamma(eta, theta, a, delta_gam)
        f_theta_dbeta = -f_theta_dsig
        matrices = (
            f_theta_dsig,
            f_theta_dtheta,
            f_theta_dbeta,
    ...
    def calc_f_beta(self, eta, beta, delta_gam):
        return (
            beta
            - self.beta
            - 2 / 3 * (self.k * self.b * eta - self.k * self.sig_y * beta) * delta_gam
        )

    def calc_j_f_beta(self, eta, beta, delta_gam):
        f_beta_dsig = -2 / 3 * self.k * self.b * delta_gam * I_sh
        f_beta_dtheta = -f_beta_dsig
        f_beta_dbeta = (
            1
            + 2 / 3 * self.k * self.b * delta_gam
            + 2 / 3 * self.k * self.sig_y * delta_gam
        ) * I_sh
        f_beta_dgamma = 2 / 3 * (self.k * self.sig_y * beta - self.k * self.b * eta)
        matrices = (
            f_beta_dsig,
            f_beta_dtheta,
            f_beta_dbeta,
    ...
    def calc_jacobian(
        ...
        j_f_beta = self.calc_j_f_beta(eta, beta, delta_gam)
        matrices = (j_f_ep, j_f_theta, j_f_beta, j_f_f)
        return np.vstack(matrices)

    def calc_f_vector(self, sig, sig_tri, beta, theta, a, delta_gam):
        eta = sig - beta - theta
        f_ep = self.calc_f_ep(sig, eta, sig_tri, delta_gam)
        f_beta = self.calc_f_beta(eta, beta, delta_gam)
        f_theta = self.calc_f_theta(eta, theta, a, delta_gam)
        f_f = self.calc_f_f(eta)
        vectors = (f_ep, f_theta, f_beta, [f_f])
        return np.hstack(vectors)

    def divide_delta_vector(self, delta_vector):
        delta_sig, delta_theta, delta_beta, delta_gam_l = np.split(
            delta_vector, (3, 6, 9)
        )
        delta_gam = delta_gam_l[0]
        return delta_sig, delta_theta, delta_beta, delta_gam

    def return_mapping(self, sig, sig_tri):
        delta_vector = np.zeros(10)
        delta_gam_i = 0.0
        eta_tri = sig_tri - self.theta - self.beta
        f_tri = self.calc_f_f(eta_tri)
        hardening_flag = True
        if f_tri > 0.0:
            logger.debug("Plastic behavior")
            ...
            for inew in range(self.RM_I):
                ...
                d_delta_vector = np.linalg.solve(jacobian, f_vector)
                delta_vector -= np.array(d_delta_vector).flatten()
                delta_sig, delta_theta, delta_beta, delta_gam_i = (
                    self.divide_delta_vector(delta_vector)
                )
                s = 1 / (1 + 2 / 3 * self.k * self.sig_y * delta_gam_i)
                sig_i = sig + delta_sig
                beta_i = self.beta + delta_beta
                theta_i = self.theta + delta_theta
                xi_n = beta_i - self.q
                ...
                if diff < self.TOL:
                    if delta_gam_i < 0.0:
                        raise ValueError("Delta gamma is negative value.")
                    logger.debug(f"Return map converged itr.{inew+1}")
                    logger.debug(f"Delta gamma: {delta_gam_i}")
                    return delta_vector, jacobian
            else:
                logger.warning("Return mapping isn't converged.")
                quit()
                raise NotConvergedError
    ...
    def integrate_stress(self, eps, del_eps):
        eps_tri = eps + del_eps
        sig = self.elastic.De_sh @ (eps - self.eps_p)
        eps_e_tri = eps_tri - self.eps_p
        sig_tri = self.elastic.De_sh @ eps_e_tri
        delta_vector, jacobian = self.return_mapping(sig, sig_tri)
        delta_sig, delta_theta, delta_beta, delta_gam = self.divide_delta_vector(
            delta_vector
        )
        if delta_gam == 0.0:
            return sig_tri, self.elastic.De_sh
        sig = sig + delta_sig
        beta = self.beta + delta_beta
        theta = self.theta + delta_theta
        eta = sig - beta - theta
        flow = P @ eta
        self.update_i(delta_beta, delta_theta, delta_gam, eta)
        j_inv = np.array(np.linalg.inv(jacobian))
        Dep = j_inv[:3, :3]
        return sig, Dep
