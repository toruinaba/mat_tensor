...
class Calculator3D_sh:
    ...
    def __init__(
        self,
        material: Material_expression_base_sh,
        goal_sig: np.ndarray,
        init_delta_t=0.01,
        min_delta_t=1.0e-05,
        max_delta_t=0.01,
    ...
    def calc_increment(self, goal):
        ...
        logger.debug(f"Goal sig: {goal}")
        logger.debug(f"Current sig: {self.sig}")
        ...
        for s_itr in range(self.NW_I):
            ...
            logger.info(f"Difference norm: {sig_diff_norm}")
    ...
    def calculate_steps(self, is_init=True):
        ...
        while self.current_time < 1.0:
            ...
            if hasattr(self.material, "eps_e_33"):
                eps_e_33 = self.material.eps_e_33
            else:
                eps_e_33 = 0.0
