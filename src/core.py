...
from src.axisymmetry.utils import Id_s_ax
from src.shell.utils import Id_s_sh
from src.material import Material_expression_base
from src.axisymmetry.materials import Material_expression_base_ax
from src.shell.materials import Material_expression_base_sh
...
class Calculator3D_ax:
    TOL = 1.0e-06
    NW_I = 20
    TOTAL_TIME = 1.0
    MAX_INCREMENT = 1000

    def __init__(self, material: Material_expression_base_ax, goal_sig: np.ndarray, init_delta_t=0.01, min_delta_t=1.0e-05, max_delta_t=0.01):
        self.material = material
        self.goal_sig = goal_sig
        self.init_delta_t = init_delta_t
        self.min_delta_t = min_delta_t
        self.max_delta_t = max_delta_t
        self.material.initialize()
        self.eps = np.array([0.0]*4)
        self.sig = np.array([0.0]*4)
        self.output = Output_data_ax()
...
class Calculator3D_sh:
    TOL = 1.0e-06
    NW_I = 20
    TOTAL_TIME = 1.0
    MAX_INCREMENT = 1000

    def __init__(self, material: Material_expression_base_sh, goal_sig: np.ndarray, init_delta_t=0.01, min_delta_t=1.0e-05, max_delta_t=0.01):
        self.material = material
        self.goal_sig = goal_sig
        self.init_delta_t = init_delta_t
        self.min_delta_t = min_delta_t
    ...
    def calc_increment(self, goal):
        f_sig = goal - self.sig
        del_eps = np.linalg.inv(self.material.elastic.De_sh) @ f_sig
        logger.info(f"Goal sig: {goal}")
        logger.info(f"Current sig: {self.sig}")
        logger.debug(f"Initial dela eps: {del_eps}")
        for s_itr in range(self.NW_I):
    ...
    def calculate_steps(self, is_init=True):
        ...
        while self.current_time < 1.0:
            ...
            if self.current_inc >= self.MAX_INCREMENT:
                raise NotConvergedError("Maximum increment is exceeded.")
            self.output.add_data(self.sig, self.eps, self.material.eps_e_33, self.material.eps_p, self.material.eff_eps_p)
            logger.info(f"Ended time: {self.current_time}")


class Output_data_sh:
    def __init__(self):
        self.initialize()
    ...
    def add_data(self, sig, eps, eps_33, eps_p, eff_eps_p):
        self.sig.append(sig)
        self.eps.append(eps)
        self.eps_33.append(eps_33)
        self.eps_p.append(eps_p)
        self.eff_eps_p.append(eff_eps_p)
        self.mises.append(self.calc_mises(sig))
